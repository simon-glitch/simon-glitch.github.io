    ...
    try{
        ...
        try{
            ...
            try{
                ...
                alert("Something broke!"); return false;
            }
        }
    }
    if(callback){
...
G.Launch = function(){
    ...
    G._Init = function(){
        ...
        if(!G.local && !INFRAME){
            window.cookieconsent_options = {
                "message": "This website uses cookies for ads and traffic analysis.", "dismiss": "Got it!", "learnMore": "Learn more", "link": "http://orteil.dashnet.org/cookieconsentpolicy.html", "target": "_blank", "theme": "http://orteil.dashnet.org/cookieconsent.css", "domain": "dashnet.org"
            };
        ...
        AddEvent(document, 'mousedown', function(event){
            G.mouseDown = true; G.mousePressed = true; G.mouseDragFrom = event.target; G.mouseDragFromX = G.mouseX; G.mouseDragFromY = G.mouseY;
        });
        AddEvent(document, 'mouseup', function(event){
            G.mouseUp = true; G.mouseDragFrom = 0;
        });
    ...
    G.button = function(obj){
        ...
        //obj can have text, tooltip (text that shows on hover), onclick (functionexecuted when button is clicked), classes (CSS classes added to the button), id (force button to have that id)
        ...
        if(obj.onclick || obj.tooltip){
            G.pushCallback(function(id, obj){
                return function(){
                    if(query(id)){
                        if(typeof obj.tooltip === 'function') G.addTooltip(query(id), {
                            func: obj.tooltip
                        });
                        else G.addTooltip(query(id), {
                            text: obj.tooltip
                        });
                        if(obj.onclick) query(id).onclick = obj.onclick;
                    }
                }
            }(id, obj));
        }
    ...
    G.updateTextTimer = function(id, func, freq){
        var el = query('updatabletextspan-' + id);
        if(el){
            var delay = freq * 1000;
            if(freq < 0){
                freq = -freq; delay = 100;
    ...
    G.tooltipped = function(text, obj, style){
        ...
        G.pushCallback(function(id, obj){
            return function(){
                if(typeof obj === 'string') G.addTooltip(query(id), {
                    text: obj
                });
                else G.addTooltip(query(id), obj);
            }
        }(id, obj));
        G.tooltipdN++;
        return str;
    ...
    G.Loop = function(){
        ...
        G.accumulatedDelay = Math.min(G.accumulatedDelay, 1000 * 5);//don't compensate over 5 seconds; ifyou do, something's probably very wrong
        G.time = new Date().getTime();
        ...
        if(INFRAME && !EDITOR){
            query('fpsCounter').innerHTML = G.currentFps + ' fps';
            var ctx = G.fpsGraphCtx;
...
if(!INFRAME){
    window.onload = function(){
        if(!G.ready){
            ...
            G.vars = vars;
            if(G.vars.e){
                query('code').innerHTML = '<iframe id="editor" src="editor.html"></iframe>';
                query('wrap').classList.add('withCode');
            }
            if(G.vars.g){
                query('game').innerHTML = '<iframe id="player" src="player.html"></iframe>';
            }
            else{
                query('game').innerHTML = '<iframe id="main" src="main.html"></iframe>';
            }
        }
    };
    ...
    window.addEventListener('message', function(e){
        if(e.data.playerReady && !G.loadedPlayer){
            G.loadedPlayer = true;
            query('player').contentWindow.postMessage({
                launch: true, loc: window.location.origin, vars: G.vars
            }, '*');
        }
        else if(e.data.editorReady){
        }
        else if(e.data.code && !G.loadedEditor && query('editor')){
            G.loadedEditor = true;
            query('editor').contentWindow.postMessage({
                code: e.data.code
            }, '*');
        }
            ...
            setTimeout(function(){
                query('player').contentWindow.postMessage({
                    launch: true, loc: window.location.origin, vars: G.vars, code: e.data.refresh
                }, '*');
            }, 100);
        }
    });
