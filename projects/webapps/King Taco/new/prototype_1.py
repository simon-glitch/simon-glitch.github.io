...
def play(card: int):
    ...
    if(symbols_d[card] in specials):
        # print("effect:", res)
        if(my_effect.length == 0 or my_effect.can_stack(card)):
    ...
    if(my_effect.length > 0):
        ...
        if(not my_effect.can_stack(card)): my_effect.clear()
