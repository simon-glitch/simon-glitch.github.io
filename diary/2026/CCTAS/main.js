/*
This is a fork of Cookie Clicker. The forking process was done by Simanelix.

Cookie Clicker's license:
----
...
* Code should hopefully be more clean now.
* Should have the same behavior as the original game.
...
let DataDir='';//'//orteil.dashnet.org/data/';
DataDir=window.location.origin+'/data/';

let getJson=function(url,callback,error)
{
	if (Game.local) return false;
	let xhr=new XMLHttpRequest();
	if (!xhr){return false;}
...
let prefixes=['','Un','Do','Tr','Qa','Qi','Sx','Sp','Oc','No'];
let suffixes=['D','V','T','Qa','Qi','Sx','Sp','O','N'];
for (let i in suffixes)
{
	for (let ii in prefixes)
	{
...
let SimpleBeautify=function(val)
{
	...
	let str2='';
	let len=str.length;
	for (let i=0;i<len;i++)//add commas
	{
...
function fixScrollbars()
{
	...
	let el=l('scrollbarFixer');
	let scrollbarWidth=Math.max(0,el.offsetWidth-el.clientWidth);
...
let LocalizeUpgradesAndAchievs=function()
{
	...
	for (let i=0;i<allThings.length;i++)
	{
		...
		let desc=it.baseDesc.replace(/<q>.*/,'');//strip quote section
		let quote=it.baseDesc.match(/<q>(.*?)<\/q>/i);//get quote section
		...
		for (let ii=0;ii<LangReplaceAlls.length;ii++)
		{
			let pair=LangReplaceAlls[ii];
...
function AddEvent(el,ev,func,capture)
{
	...
	else if (el.attachEvent) {let func2=function(){func.call(el);};el.attachEvent('on'+ev,func2);return [el,ev,func2,capture];}
	return false;
}
...
let PlaySound=function(url,vol,pitchVar)
{
	...
	else if (Sounds[url].readyState>=2 && SoundInsts[SoundI].paused)
	{
		...
		if (pitchSupport)
		{
			let pitchVar=(typeof pitchVar==='undefined')?0.05:pitchVar;
...
Game.Launch=function() {
	...
	let preloadImages=['img/goldCookie.png','img/wrathCookie.png','img/familiars.png','img/hearts.png','img/spamCookies.gif','img/bunnies.png','img/frostedReindeer.png','img/goldCookieWreath.png','img/wrathCookieWreath.png'];
	...
	Game.Init=function()
	{
		...
		Game.resize=function()
		{
			...
			*/
			...
			for (let i=0;i<Game.onResizeCallbacks.length;i++){Game.onResizeCallbacks[i](w,h);}
		}
		Game.resize();
		...
		{
			if (Game.bakeryName!=what && what=='harley'){let rect=Game.bakeryNameL.getBounds();Game.Popup('Horse complex!',(rect.left+rect.right)/2,(rect.top+rect.bottom)/2-48);}//added by request as thanks to the very nice person who kept my twitter handle safe when my account got hacked by crypto scammers in 2024
		...
		Game.CheckUpdates=function()
		{
			if (!App) getJson(DataDir+'version.json',function(r){
				let entry='Cookie Clicker'+(Game.beta?' beta':'');
				if (r && r[entry])
				{
					let data=r[entry];
					let str='';
		...
		if (!App)
		{
			...
			getJson(DataDir+'cookieclickerinfo.json',function(r){
				if (r)
				{
					...
					let url=new URL(window.location.href);url=url.origin+url.pathname.replace(/\/$/,'');
			...
			if (str!='')
			{
				...
				else
				{
					...
					if (version>=1)
					{
						...
						{
							let me=Game.Upgrades['Persistent memory'];me.unlocked=0;me.bought=0;
							let me=Game.Upgrades['Season switcher'];me.unlocked=0;me.bought=0;
						}
						...
						if (Game.mobile || Game.Has('Perfect idling') || Game.Has('Twin Gates of Transcendence'))
						{
							...
							{
								let maxTime=60*60*24*1000000000;
								let percent=100;
							}
							else
							{
				...
				if (me.spawnsOnTimer && me.spawnConditions())//only run on shimmer types that work on a timer
				{
					if (!me.spawned)//no shimmer spawned for this type? check the timer and try to spawn one
					{
						...
						if (Math.random()<Math.pow(Math.max(0,(me.time-me.minTime)/(me.maxTime-me.minTime)),5))
						{
							...
							if (Game.Has('Distilled essence of redoubled luck') && Math.random()<0.01) let newShimmer=new Game.shimmer(i);
							me.spawned=1;
						}
					}
				}
		...
		Game.shimmerTypes={
			...
			'golden':{
				...
				initFunc:function(me)
				{
					...
					//set image
					let bgPic=Game.resPath+'img/goldCookie.png';
					let picX=0;let picY=0;
					let picW=96;let picH=96;
		...
		Game.particlesDraw=function(z)
		{
			...
			for (let i=0;i<Game.particlesN;i++)
			{
				...
				if (me.z==z)
				{
					if (me.life!=-1)
					{
						let opacity=Math.pow(1-(me.life/(Game.fps*me.dur)),0.5);
		...
		{
			let el=l('promptOption'+Game.promptOptionFocus);
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			for (let i=0;i<Game.modHooks['tickerFinal'].length;i++)
			{
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			if (this.n!=0)
			{
				...
				let digits=Math.pow(10,(Math.ceil(Math.log(Math.ceil(this.basePrice))/Math.LN10)))/100;
			...
			else
			{
				...
				this.draw=function()
				{
					...
					*/
					...
					{
						let pic=this.pics[i];
						let sprite=pic.pic=='canvasAdd'?this.canvasAdd:Pic(pic.pic);
						...
						if (selected==i && this.name=='You')
						{
							...
							let cloneTitle=Math.floor(Math.random()*3);
							Math.seedrandom(Game.seed+' clone '+pic.id);
							ctx.font='9px Merriweather';
							ctx.textAlign='center';
							let text=loc("Clone")+' #'+Math.floor(Math.random()*500+pic.id*500+1);
							if (EN)
							{
								text=[
									text,
							...
							let width=ctx.measureText(text).width+12;
							let x=Math.max(0,Math.min(pic.x+32-width/2,this.canvas.width-width));
							let y=10;
							ctx.fillStyle='#000';
					...
					let w=1;
					let h=1;
					let w=Math.abs(Math.cos(Game.realT*0.2+this.id*2-0.3))*0.2+0.8;
					let h=Math.abs(Math.sin(Game.realT*0.2+this.id*2))*0.3+0.7;
					let x=64+Math.cos(Game.realT*0.19+this.id*2)*8-24*w;
					let y=128-Math.abs(Math.pow(Math.sin(Game.realT*0.2+this.id*2),5)*16)-48*h;
		...
		{
			let me=Game.Objects['You'];
			let ctx=me.ctxAdd;
			let img='you.png';
			let imgAddons='youAddons.png?v='+Game.version;
			...
			Game.Loader.waitForLoad([img,imgAddons],function(){
				...
				{
					ctx.drawImage(Pic(img),0,0);
					let canvasAddon=document.createElement('canvas');
					canvasAddon.width=32;
					canvasAddon.height=32;
					ctxAddon=canvasAddon.getContext('2d');
					let canvasCols=document.createElement('canvas');
					let colsN=64;
					canvasCols.width=8;
					canvasCols.height=colsN;
					let ctxCols=canvasCols.getContext('2d');
					ctxCols.drawImage(Pic(imgAddons),0,0,8,colsN,0,0,8,colsN);
					let imgDataCols=ctxCols.getImageData(0,0,8,colsN);
					let dataCols=imgDataCols.data;
					let cols=[];
					...
					let colSkinFull=[[32,14,10],[180,80,54],[208,144,101],[225,192,150]];
					let colSkin=[];for (let colI=0;colI<colSkinFull.length;colI++){colSkin[colI]=colSkinFull[colI][0]*1000000+colSkinFull[colI][1]*1000+colSkinFull[colI][2];}
					let colHairFull=[[32,14,10],[82,55,53],[100,83,80],[116,97,89]];
					let colHair=[];for (let colI=0;colI<colHairFull.length;colI++){colHair[colI]=colHairFull[colI][0]*1000000+colHairFull[colI][1]*1000+colHairFull[colI][2];}
					let shade1=0*1000000+118*1000+206;
					let shade2=0*1000000+71*1000+125;
					
					//apply addon canvases to main canvas, handling shading on skin and hair where necessary
					let addonGenes=['face','head','hair','acc1','acc2'];
					for (let geneI=0;geneI<addonGenes.length;geneI++)
					{
						let addonTile=Game.YouCustomizer.getGeneValue(addonGenes[geneI]);
						ctxAddon.clearRect(0,0,32,32);
						ctxAddon.drawImage(Pic(imgAddons),8+addonTile[0]*32,addonTile[1]*32,32,32,0,0,32,32);
						
						let imgDataAddon=ctxAddon.getImageData(0,0,32,32);
						let dataAddon=imgDataAddon.data;
						let x=0;let y=0;
						for (i=0;i<dataAddon.length;i+=4)
						{
							let r=dataAddon[i];let g=dataAddon[i+1];let b=dataAddon[i+2];let a=dataAddon[i+3];
							...
							if (a!=0)
							{
								...
								let go=data[off+1];
								let bo=data[off+2];
								let col=r*1000000+g*1000+b;
								let shade=col==shade2?2:col==shade1?1:0;
								let indShadeAddon=colSkin.indexOf(r*1000000+g*1000+b);
								let indShadeOr=colSkin.indexOf(ro*1000000+go*1000+bo);
								let typeOr=0;
								...
								{//light blue: shade one stage; dark blue: shade 2 stages
									let colOut=(typeOr==1?colSkinFull:typeOr==2?colHairFull:0)[Math.max(0,indShadeOr-shade)];
					...
					let skinCol=Game.YouCustomizer.getGeneValue('skinCol');
					let hairCol=Game.YouCustomizer.getGeneValue('hairCol');
					for (i=0;i<data.length;i+=4)
					{
						let r=data[i];let g=data[i+1];let b=data[i+2];let a=data[i+3];
						if (a!=0)
						{
							let indSkin=colSkin.indexOf(r*1000000+g*1000+b);
							if (indSkin>0)
							{
								let col=cols[skinCol][indSkin-1];
								data[i]=col[0];data[i+1]=col[1];data[i+2]=col[2];
							}
							else
							{
								let indHair=colHair.indexOf(r*1000000+g*1000+b);
								if (indHair>0)
								{
									let col=cols[hairCol][indHair-1];
									data[i]=col[0];data[i+1]=col[1];data[i+2]=col[2];
								}
							}
						}
		...
		Game.YouCustomizer.genesById={};for (let i=0;i<Game.YouCustomizer.genes.length;i++){Game.YouCustomizer.genes[i].n=i;Game.YouCustomizer.genesById[Game.YouCustomizer.genes[i].id]=Game.YouCustomizer.genes[i];}
		...
		{
			let gene=Game.YouCustomizer.genesById[id];
		...
		Game.YouCustomizer.resetGenes=function(){for (let i=0;i<Game.YouCustomizer.genes.length;i++){Game.YouCustomizer.currentGenes[i]=Game.YouCustomizer.genes[i].def;}}
		Game.YouCustomizer.resetGenes();
		...
		Game.YouCustomizer.randomize=function()
		{
			for (let i=0;i<Game.YouCustomizer.genes.length;i++)
			{
				let gene=Game.YouCustomizer.genes[i];
		...
		Game.YouCustomizer.renderPortrait=function()
		{
			...
			let ctx=l('youCustomizerPreview').getContext('2d');
		...
		{
			let makeCustomizerSelector=function(gene,text)
			{
				gene=Game.YouCustomizer.genesById[gene];
		...
		{
			let building=Game.Objects[building];
		...
		let desc=function(percent,total){return loc("You gain another <b>+%1%</b> of your regular CpS while the game is closed, for a total of <b>%2%</b>.",[percent,total]);}
		...
		if (EN)
		{
			Game.last.descFunc=function(){
				...
				let n=35;
				let i=Math.floor(Game.realT*0.1);
		...
		Game.last.choicesFunction=function()
		{
			...
			if (App)
			{
				...
				for (let i=0;i<Game.jukebox.tracks.length;i++)
				{
		...
		Game.promptGiftRedeem=function()
		{
			...
			let checkCode=function(str)
			{
				let out={cookies:1,message:false,icon:Game.giftBoxDesigns[0]};
				str=b64_to_utf8(str);
				if (!str) return false;
				str=str.split('|');
				if (str[0]!=='MAIL') return false;
				
				let val=parseInt(str[1]||0);
				...
				val=(str[4]||'').split('\n').slice(0,4);
					for (let i=0;i<val.length;i++){val[i]=val[i].substring(0,25);}
					val=val.join('\n');
					val=val.replace(/\/\$\//g,'|');
			...
			(function(checkCode){
				let inputCode=function(){
					let val=l('giftCode').value;
					
					let disabled=true;
					if (val && val.length>5)
					{
						let out=checkCode(val);
				...
				l('promptOption0').addEventListener('click',function(){
					...
					let out=checkCode(l('giftCode').value);
		...
		Game.promptGiftSend=function()
		{
			...
			'</div>',[[loc("Cancel"),0,'float:right']],function(){
				if (!l('giftPromptContent'))
				{
					...
					let icon=Game.giftBoxDesigns[0];
					...
					l('giftBoxDesignButton').addEventListener('click',function(){
						...
						for (let i=0;i<icons.length;i++)
						{
							l('giftSelector-'+i).addEventListener('click',function(e){
								let icon=e.target.dataset.icon.split(' ');icon=[parseInt(icon[0]),parseInt(icon[1])];
								l('giftBoxDesign').dataset.icon=icon[0]+' '+icon[1];
								l('giftBoxDesign').style.backgroundPosition=(-icon[0]*48)+'px '+(-icon[1]*48)+'px';
					...
					let func=function(){
						...
						for (let i=0;i<icons.length;i++)
						{
							let icon=icons[i].split(' ');icon=[parseInt(icon[0]),parseInt(icon[1])];
					...
					let inputAmount=function(){
						let val=parseInt(l('giftAmount').value||0);
						if (val<1) val=1;
						if (val>1000) val=1000;
						val=val||1;
					...
					l('promptOption0').addEventListener('click',function(){
						...
						let val=l('giftBoxDesign').dataset.icon;
						if (!val || val=='none') val='-';
		...
		Game.keepsakes=[];
		for (let i=0;i<Game.seasonDrops.length;i++){Game.keepsakes.push(Game.seasonDrops[i]);}
		...
		Game.BankAchievement=function(name,q)
		{
			...
			let achiev=new Game.Achievement(name,loc("Bake <b>%1</b> in one ascension.",loc("%1 cookie",{n:threshold,b:toFixed(threshold)}))+(q?('<q>'+q+'</q>'):''),[Game.thresholdIcons[Game.BankAchievements.length],(Game.BankAchievements.length>45?0:Game.BankAchievements.length>43?2:Game.BankAchievements.length>32?1:Game.BankAchievements.length>23?2:5)]);
		...
		Game.CpsAchievement=function(name,q)
		{
			...
			let achiev=new Game.Achievement(name,loc("Bake <b>%1</b> per second.",loc("%1 cookie",{n:threshold,b:toFixed(threshold)}))+(q?('<q>'+q+'</q>'):''),[Game.thresholdIcons[Game.CpsAchievements.length],(Game.CpsAchievements.length>45?0:Game.CpsAchievements.length>43?2:Game.CpsAchievements.length>32?1:Game.CpsAchievements.length>23?2:5)]);
			achiev.threshold=threshold;
		...
		let order=0;//this is used to set the order in which the items are listed
		...
		order=61000;
		let achiev=new Game.Achievement('Getting even with the oven',EN?'Defeat the <b>Sentient Furnace</b> in the factory dungeons.':'???',[12,7]);Game.last.pool='dungeon';
		let achiev=new Game.Achievement('Now this is pod-smashing',EN?'Defeat the <b>Ascended Baking Pod</b> in the factory dungeons.':'???',[12,7]);Game.last.pool='dungeon';
		let achiev=new Game.Achievement('Chirped out',EN?'Find and defeat <b>Chirpy</b>, the dysfunctionning alarm bot.':'???',[13,7]);Game.last.pool='dungeon';
		let achiev=new Game.Achievement('Follow the white rabbit',EN?'Find and defeat the elusive <b>sugar bunny</b>.':'???',[14,7]);Game.last.pool='dungeon';
		...
		*/
		...
		Game.DrawWrinklers=function()
		{
			...
			for (let i in Game.wrinklers)
			{
				...
				if (me.phase>0)
				{
					...
					ctx.save();
					ctx.translate(me.x,me.y);
					let sw=100+2*Math.sin(T*0.2+i*3);
					let sh=200+5*Math.sin(T*0.2-2+i*3);
					if (Game.prefs.fancy)
					{
						ctx.translate(0,30);
		...
		for (let i in Game.santaLevels){Game.santaLevels[i]=loc(Game.santaLevels[i]);}
		...
		Game.ToggleSpecialMenu=function(on)
		{
			if (on)
			{
				...
				let str='<div id="specialPic" '+Game.clickStr+'="Game.ClickSpecialPic();" style="'+((Game.specialTab=='dragon' && Game.dragonLevel>=4 && Game.Has('Pet the dragon'))?'cursor:pointer;':'')+'position:absolute;left:-16px;top:-64px;width:96px;height:96px;background:url('+Game.resPath+'img/'+pic+');background-position:'+(-frame*96)+'px 0px;filter:drop-shadow(0px 3px 2px #000);-webkit-filter:drop-shadow(0px 3px 2px #000);"></div>';
		...
		Game.DrawSpecial=function()
		{
			...
			for (let i in Game.specialTabs)
			{
				...
				{
					let ss=s*64;
					let r=Math.floor((Game.realT*0.5)%360);
		...
		Game.MakeMilk=function()
		{
			...
			let cols=['#6D84DA','#D8BFA9','#E6E9C3'];//plain milk
			let cols=['#FF4C0E','#553626','#C0784A'];//chocolate milk
			let cols=['#810600','#D61643','#C63324'];//cherry milk
			let bubbles=false;
			
			let pics=['milkMask.png','milkDarkMask.jpg','milkCol1Mask.jpg','milkCol2Mask.jpg'];
		...
		Game.DrawBackground=function()
		{
			...
			if (Game.OnAscend)
			{
				...
				let s=1*Game.AscendZoom*(1+Math.cos(Game.realT*0.0027)*0.05);
				...
				if (Game.prefs.fancy)
				{
					...
					let s=2*Game.AscendZoom*(1+Math.sin(Game.realT*0.002)*0.07);
			...
			else
			{
				...
				if (Game.AscendTimer==0)
				{
					if (Game.prefs.particles)
					{
						...
						if (pic!='')
						{
							...
							ctx.globalAlpha=opacity;
							let y=(Math.floor(Game.realT*2)%512);
						...
						{
							let y=(Math.floor(Game.realT*2.5)%512);
							ctx.globalAlpha=0.75;
							ctx.globalCompositeOperation='lighter';
						...
						{
							//big dragon
							let s=300*2*(1+Math.sin(Game.realT*0.013)*0.1);
							let x=Game.cookieOriginX-s/2;
							let y=Game.cookieOriginY-s/(1.4+0.2*Math.sin(Game.realT*0.01));
				...
				else
				{
					...
					//flares
					let n=9;
					let t=Game.AscendTimer/Game.AscendBreakpoint;
				...
				if (Game.prefs.milk)
				{
					...
					let x=Math.floor((Game.realT*2-(Game.milkH-Game.milkHd)*2000+480*2)%480);//Math.floor((Game.realT*2+Math.sin(Game.realT*0.1)*2+Math.sin(Game.realT*0.03)*2-(Game.milkH-Game.milkHd)*2000+480*2)%480);
					let y=(Game.milkHd)*height;//(((Game.milkHd)*ctx.canvas.height)*(1+0.05*(Math.sin(Game.realT*0.017)/2+0.5)));
					let a=1;
					if (Game.AscendTimer>0)
					{
				...
				if (Game.AscendTimer==0)
				{
					...
					if (Game.Has('Shimmering veil [off]'))
					{
						ctx.globalAlpha=1;
						ctx.globalCompositeOperation='lighter';
						let s=300+Math.sin(Game.realT*0.037)*20;
						let x=Game.cookieOriginX;
						let y=Game.cookieOriginY;
						ctx.save();
						ctx.translate(x,y);
						...
						if (Game.prefs.particles)//sparkles
						{
							for (i=0;i<6;i++)
							{
								let t=Game.T+i*15;
								let r=(t%30)/30;
								let a=(Math.floor(t/30)*30*6-i*30)*0.01;
								let size=32*(1-Math.pow(r*2-1,2));
								let xx=x+Math.sin(a)*(110+r*16);
								let yy=y+Math.cos(a)*(110+r*16);
		...
		if (!App && Game.local && new URL(window.location.href).searchParams.get('addon'))
		{
			...
			let path=new URL(window.location.href).searchParams.get('addon');
	...
	Game.Draw=function()
	{
		...
		if (!Game.OnAscend)
		{
			...
			{
				let pulse=Math.pow((Game.realT%10)/10,0.5);
	...
	Game.Loop=function()
	{
		...
		let startTime=Date.now();//adapted from a mod called FPSTweaker
		Timer.say('START');
		...
		let loadLangAndLaunch=function(lang,firstLaunch)
		{
			...
			let launchSteps=[
				...
				function(next){
					...
					{
						let mods=PRESETMODS;
						let modsN=mods.length;
						let modsLoadedN=0;
						let modLoaded=function()
						{
							modsLoadedN++;
							if (modsLoadedN>=modsN) next();
						};
						let modError=function()
			...
			let doLaunchStep=function(step)
			{
				if (!launchSteps[step]) return;
				let next=function(){doLaunchStep(step+1);};
