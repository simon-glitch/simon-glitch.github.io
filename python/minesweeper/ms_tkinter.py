...
def force_zero(pos: list):
  ...
  for i0 in range(3):
    ...
    for i1 in range(3):
      ...
      # print("checked @ ", j0, ",", j1, "with i0 , i1 =", i0, ",", i1, "; just for fun!")
  ...
  print("new hint @", pos, ":", field.hints[pos[0]][pos[1]])
...
def tk_generate():
  ...
  for iy in range(bh):
    ...
    for ix in range(bw):
      ...
      field.hints[iy][ix])
      btn_text = tk.StringVar(tk_screen, row_ss)
      btns_text[iy].append(btn_text)
      
      # Make the button
      #   tk.Button makes a button, but doesn't actually add it to the display window
      btn = tk.Button(tk_screen, textvariable = btn_text, command = button_handler_generator(ix, iy))
