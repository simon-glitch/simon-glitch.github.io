...
body = document.body
html_field = document.createElement("div")
html_field.className = "field"
body.appendChild(html_field)


# a = 54
# def contextmenu_f(b):
#   return a + b
# document.body.oncontextmenu = contextmenu_f
...
def print_cell(on_cursor, rdii, raii, rhii, rfii):
  row_ss = ""
  row_ss += "<" if on_cursor else ""
  row_ss += (
    ("⛰️") if ((not rdii) and (not rfii)) else (
      (
        ("🚩") if (rfii) else
        ("💣")
      ) if
...
def display():
  ...
  if(field.doprint): print(f"{remaining} mines remaining:")
  
  
  def p_row(i, row_has, row_hint, row_discovered, width, row_flagged):
    row_s = ""
    for ii in range(width):
      on_cursor = (i == field.cursor[0]) and (ii == field.cursor[1])
      row_ss = print_cell(on_cursor, row_discovered[ii], row_has[ii], row_hint[ii], row_flagged[ii])
      row_s += row_ss
  ...
  if(use_terminal_display):
    for i in range(height):
      ...
      field.discovered[i], width, field.flagged[i])
    )
...
def clear_zeros():
  ...
  def clear_at(i,ii,testing):
    ...
    if (
      (hints[i][ii] > 0) or
      (testing and (not discovered[i][ii])) or
      (not(testing or (not discovered[i][ii]))) or
      scanned[i][ii]
    ):
      return False
...
# put simply, this attempts to make a discovery at the current location of the cursor
def attempt_discovery(s: str):
  c       = field.cursor
...
def process_input(s):
  ...
  if(field.doprint): print("now: ", t_now.strftime("%m/%d/%Y, %H:%M:%S"))
  if(field.doprint): print(f"elpased: {diff:.1f} seconds")
...
def start_game(use_input):
  ...
  if((not use_input) or input("type `p` to play~") == "p"):
    ...
    playing = use_input
    while playing:
      playing = play_turn_with_terminal()
...
def html_generate():
  ...
  # btn_rows should be the same length as btns, so... this should just work
  while (len(btns) > 0):
    btns.pop()
    btn_row = btn_rows.pop()
    btn_row.remove()
  
  for iy in range(bh):
    btns.append([])
    btn_row = document.createElement("p")
    btn_row_con = document.createElement("p")
    btn_row_con.className = "wrapper"
    html_field.appendChild(btn_row_con)
    btn_row_con.appendChild(btn_row)
    ...
    for ix in range(bw):
      ...
      # mixing JS with Python sure is a lot of fun!
      btn_row.appendChild(btn)
      command = button_handler_generator(ix, iy)
      # onclick?
      btn.onclick = command
...
def html_update():
  bw = field.width
  bh = field.height
  # automatically resize the field as necessary
  if(
    # check if there are any buttons 1st, so we can initialize the buttons for the 1st game
    len(btns) == 0 or
    # then check that the current size matches what we are expecting
    len(btns) != bh or
    len(btns[0]) != bw
  ):
    html_generate()
  ...
  for iy in range(bh):
    for ix in range(bw):
      # update button text, using the same code used to print cells in the terminal
      on_cursor = (iy == field.cursor[0]) and (ix == field.cursor[1])
      row_ss = print_cell(on_cursor, field.discovered[iy][ix], field.has_mine[iy][ix], field.hints[iy][ix], field.flagged[iy][ix])
      # trivial
      btns[iy][ix].innerText = row_ss
  return True

# key detection
def keydown_f(event):
  c = event.key
  # replace arrows with w-a-s-d
  if(c == "ArrowUp"): c = "w"
  if(c == "ArrowLeft"): c = "a"
  if(c == "ArrowDown"): c = "s"
  if(c == "ArrowRight"): c = "d"
