...
let has_mine = new Uint8Array();
const has_mine_at = function(x, y){
  const i = x + y*c.width;
  const ii = i % 8;
  const iii = (i - ii) / 8;
  return ((has_mine[iii] & (1 << ii)) >> ii);
};
...
const load_maze = function(maze){
  ...
  for(let i = 0; i < maze.length; i++){
    ...
    if(m & 16) draw(ii);
    if((odd_h || iy >   0) && (m & 1)) draw(ii + off[0]);
    if((odd_h || iy < h-1) && (m & 2)) draw(ii + off[1]);
    if((odd_w || ix >   0) && (m & 4)) draw(ii + off[2]);
    if((odd_w || ix < w-1) && (m & 8)) draw(ii + off[3]);
  }
};
