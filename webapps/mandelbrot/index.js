 ...
 * link:
 *   http://127.0.0.1:5500/projects/webapps/mandelbrot/
 */
...
SCOPE_NEWP: {
  let i,a,o,u;
  i = function(e) {
    return (e.abs().lt(s.EPSILON) ? ONE.minus(e) : ONE_HALF.times(e.exp().plus((e.neg().exp()))));
  };
  a = function(e) {
    return (e.abs().lt(s.EPSILON) ? e : ONE_HALF.times(e.exp().minus((e.neg().exp()))));
  };
  o = function() {
    ...
    return (
      ...
      : (
        ...
        : (
          ...
          ? (ONE_HALF.times((e.times(e).plus(t.times(t)).log())))
          : (
            e = e.times(ONE_HALF),
            t = t.times(ONE_HALF),
            (ONE_HALF.times((e.times(e).plus(t.times(t)).log())).plus(LN2))
          )
        )
      )
    );
...
SCOPE_ALT_NAMES: {
  ...
  s.prototype.inv  = s.prototype.inverse;
  s.prototype.eq   = s.prototype.equals;
  ...
  s.prototype.s_logb = function(that){
    return this.log().s_div(that.log());
  }
  let max_inflog = 1_000;
  s.prototype.s_inflog = function(that){
    let i = 0;
    // get 1 / log(that)
    ...
    let e = that.re;
    let t = that.im;
    // start (current that) = this
    that = this.clone();
    let prev = that;
    that = prev.log().mul(e, t);
    for(i = 0; (i < max_inflog) && (!prev.eq(that.re, that.im)); i++){
      // take log(current that), base (original that)
      prev = that;
      that = prev.log().mul(e, t);
    }
    console.log("took " + i + " steps!");
    return that;
  };
